    ...
    try:
        ...
        with open('timestampsdepth.txt', 'w') as tfile:
            for i in range(1, nframes+1, interval):
                ...
                depth_array = depth_array/np.max(np.max(depth_array))*255
                depth_array[depth_array > 255] = 255
                depth_array[depth_array < 0] = 0
                depth_array = depth_array.astype('uint8')
                
                cv2.imwrite(dst + "/" + f"{format(i, '04d')}.png", depth_array)
